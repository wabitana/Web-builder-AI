...
export default function Builder() {
  ...
  // Mobile panel state: 'preview' | 'code' | 'chat' | 'files'
    ...
    try {
      ...
      Object.entries(files).forEach(([path, data]) => {
        ...
        if (cleanPath.startsWith('server/')) {
          ...
          // It's a frontend file
          if (cleanPath === 'App.js') {
            code = code.replace(/from\s+['"]\.\/(src\/)/g, "from './");
            code = code.replace(/import\s+['"]\.\/(src\/)/g, "import './");
            cleanPath = 'client/src/App.js';
          } else if (cleanPath.startsWith('src/')) {
            cleanPath = 'client/' + cleanPath;
          } else {
            if (cleanPath.endsWith('.js') || cleanPath.endsWith('.jsx') || cleanPath.endsWith('.ts') || cleanPath.endsWith('.tsx') || cleanPath.endsWith('.css')) {
              cleanPath = 'client/src/' + cleanPath;
            } else {
              cleanPath = 'client/' + cleanPath;
            }
          }
      ...
      zip.file("client/package.json", JSON.stringify({
        name: projectName.replace(/\s+/g, '-').toLowerCase() + '-client',
        version: "0.1.0",
        private: true,
        dependencies: {
          "react": "^18.2.0",
      ...
      // Backend package.json (if backend exists)
      if (hasBackend) {
        zip.file("server/package.json", JSON.stringify({
          name: projectName.replace(/\s+/g, '-').toLowerCase() + '-server',
          version: "0.1.0",
          private: true,
          dependencies: {
            "express": "^4.18.2",
      ...
      zip.file("client/public/index.html", `<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
  ...
  <body>
    ...
    <div id="root"></div>
      ...
      zip.file("client/src/index.js", `import React from 'react';
...
root.render(
  <React.StrictMode>
    <App />
      ...
      zip.file("client/tailwind.config.js", `/** @type {import('tailwindcss').Config} */
module.exports = {
...
This project was generated by Web Builder AI.
...
`);
        ...
        {Object.keys(files).sort().map(filename => (
          <button
            ...
            onClick={() => {
              ...
              setViewMode('code');
              // On mobile, switch to code panel after selecting a file
              setMobilePanel('code');
            }}
  ...
  const renderChatPanel = () => (
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="p-3 sm:p-4 border-t border-gray-200 dark:border-white/10 flex-shrink-0">
  ...
  const renderCodePanel = () => (
    <div className="flex-1 bg-[#1e1e1e] overflow-auto">
      ...
      <pre className="p-4 sm:p-6 text-sm text-green-400 font-mono whitespace-pre-wrap leading-relaxed">
  ...
  return (
    <div className="h-screen flex flex-col bg-white dark:bg-[#050505] text-gray-900 dark:text-white transition-colors overflow-hidden font-sans">
      {/* Top Header */}
      <header className="h-14 border-b border-gray-200 dark:border-white/10 flex items-center justify-between px-2 sm:px-4 flex-shrink-0 bg-white dark:bg-[#0A0A0A]">
        <div className="flex items-center gap-2 sm:gap-3 min-w-0">
          <button onClick={() => navigate('/dashboard')} className="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-white/5 transition-colors flex-shrink-0">
            <ArrowLeft className="w-4 h-4" />
          </button>
          <div className="flex items-center gap-2 min-w-0">
            <Zap className="w-5 h-5 text-blue-600 fill-blue-600 flex-shrink-0" />
            <input
              value={projectName}
              onChange={(e) => setProjectName(e.target.value)}
              className="bg-transparent font-bold text-sm outline-none border-none w-24 sm:w-48 focus:ring-2 focus:ring-blue-500/50 rounded px-1 sm:px-2 py-1"
            />
          </div>
        </div>

        <div className="flex items-center gap-1 sm:gap-2">
          {/* Device mode selector - hidden on mobile */}
          {viewMode === 'preview' && (
            <div className="hidden lg:flex items-center border border-gray-200 dark:border-white/10 rounded-lg overflow-hidden mr-2">
              {[
          ...
          {/* View mode toggle - hidden on mobile (handled by bottom tabs) */}
          <div className="hidden md:flex items-center border border-gray-200 dark:border-white/10 rounded-lg overflow-hidden mr-2">
          ...
          <button onClick={handleUndo} disabled={historyIndex <= 0} className="hidden sm:block p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-white/5 text-gray-400 disabled:opacity-30"><Undo2 className="w-4 h-4" /></button>
          <button onClick={handleRedo} disabled={historyIndex >= history.length - 1} className="hidden sm:block p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-white/5 text-gray-400 disabled:opacity-30"><Redo2 className="w-4 h-4" /></button>
      ...
      <div className="flex-1 hidden md:flex overflow-hidden">
        ...
        <div className="flex-1 flex flex-col bg-gray-100 dark:bg-[#111] overflow-hidden relative">
          {viewMode === 'code' ? renderCodePanel() : renderPreviewPanel()}
        </div>

        {/* Right Sidebar (Chat AI) */}
        <div className="w-96 border-l border-gray-200 dark:border-white/10 flex flex-col bg-white dark:bg-[#0A0A0A] flex-shrink-0">
          {renderChatPanel()}
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col md:hidden overflow-hidden">
        {/* Active mobile panel */}
        <div className="flex-1 flex flex-col overflow-hidden bg-gray-50 dark:bg-[#080808]">
          {mobilePanel === 'chat' && (
        ...
        {/* Bottom Tab Bar */}
        <div className="flex-shrink-0 border-t border-gray-200 dark:border-white/10 bg-white dark:bg-[#0A0A0A] flex items-center justify-around py-2 px-1 safe-area-bottom">
          {[
